C  This Fortran script contains the Fast Fourier Transform of real,
C  periodic sequences and the cosine transform (using FFT) of zero
C  Neumann boundary conditions.
C
C  It can be compiled and archived in libs, then called from outside 
C  in the source codes. See readme.txt.
C  
C  The main subroutines are: VRFFTI: initialization of FFT
C                            VRFFTF: forward transform
C                            VRFFTB: backward or inverse transform
C  
C                            VCOSQI: initialization of cos transform
C                            VCOSQF: forward transform
C                            VCOSQB: backward or inverse transform
C
C  Note added by Anthony Ge (KTH) on 24 Januray 2017.    
C--------------------------------------------------------------------
...
!          integer.
!
!  R       a real two-dimensional array of size MDIMX x N containing the
...
!                     VRADB3, VRADB4, VRADB5, VRADBG, PIMACHNEW
!
!     SPECIAL         NONE
!     CONDITIONS
!
...
!***FIRST EXECUTABLE STATEMENT  VRFFTI
      ...
      FUNCTION PIMACHNEW(DUM)
!***BEGIN PROLOGUE  PIMACHNEW
!
...
!***END PROLOGUE  PIMACHNEW
!
!***FIRST EXECUTABLE STATEMENT  PIMACHNEW
      PIMACHNEW = 3.1415926535897932
      RETURN
      END FUNCTION PIMACHNEW
